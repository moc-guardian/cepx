...
dumps/<UF>.cepaberto_parte_<n>.csv, ready for load_cepaberto.py.
...
def fetch(
    ...
    query = urllib.parse.urlencode({"name": uf, "part": part})
...
def _download_one(
    ...
    """Fetch+extract+save one (uf, part). Returns (label, csv_bytes, error)."""
    label = f"{uf} part {part}"
    last_error = "unknown error"

    for attempt in range(retries + 1):
        if abort.is_set():
            return (label, None, "skipped (aborted)")

        try:
            body = fetch(uf, part, cookie, token_enc, timeout)

            if _looks_like_login_page(body):
                abort.set()  # session dead: stop the rest of the run

                return (label, None, "login page (session expired)")

            csv_bytes = _extract_csv(body)
            dest = os.path.join(out, f"{uf}.cepaberto_parte_{part}.csv")
            ...
            return (label, len(csv_bytes), None)
...
def main() -> None:
    ...
    if args.dry_run:
        for uf in ufs:
            for part in parts:
                query = urllib.parse.urlencode({"name": uf, "part": part})
                print(f"POST {_URL}?{query} -> {uf}.cepaberto_parte_{part}.csv")

        print(f"\n{len(ufs) * len(parts)} requests (dry run)")
    ...
    abort = threading.Event()
    tasks = [(uf, part) for uf in ufs for part in parts]
    total = len(tasks)
    ok = 0
    failures: list[str] = []
